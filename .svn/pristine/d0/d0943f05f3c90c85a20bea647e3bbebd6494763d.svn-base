...
public class Util {
	...
	public static AddressPres addressToAddressPres(Address address) {
		AddressPres addp = new AddressPres();
	...
	public static List<AddressPres> addressToAddressPresList(List<Address> list) {
		return list.stream().map(a -> {
			AddressPres ap = new AddressPres();
			ap.setAddress(a);
			return ap;
		}).collect(Collectors.toList());
	}
	...
	public static double computeTotal(List<CartItem> list) {
		DoubleSummaryStatistics summary = list.stream()
				.collect(Collectors.summarizingDouble(item -> Double.parseDouble(item.getTotalprice())));
