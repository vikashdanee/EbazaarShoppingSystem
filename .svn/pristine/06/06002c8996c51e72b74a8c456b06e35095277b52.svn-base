...
public enum BrowseSelectData  {
	...
	public List<CartItemPres> getSavedCart(){
		CustomerSubsystem css = DataUtil.readCustFromCache();
		ShoppingCartSubsystem scss = css.getShoppingCartSubsystem();
		List<CartItemPres> cartItemPressList = null;
		
		try {
			presentation.util.Util.cartItemsToCartItemPres(null);
			scss.retrieveSavedCart();
			scss.makeSavedCartLive();
			ShoppingCart shopingCart = scss.getLiveCart();
			cartItemPressList = presentation.util.Util.cartItemsToCartItemPres(shopingCart.getCartItems());
			
			this.cartData2 = new ArrayList<CartItemPres>();
			this.cartData2.addAll(cartItemPressList);
			
			return cartItemPressList;
